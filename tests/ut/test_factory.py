...
from pycached import SimpleMemoryCache, RedisCache, caches, Cache, CACHE_CACHES
...
class TestCache:
    ...
    def test_new_invalid_cache_raises(self):
        with pytest.raises(InvalidCacheType) as e:
            ...
            assert str(e.value) == "Invalid cache type, you can only use {}".format(
                list(CACHE_CACHES.keys())
            )

    @pytest.mark.parametrize("scheme", [Cache.MEMORY, Cache.REDIS])
    ...
    @pytest.mark.parametrize("scheme", ["memory", "redis"])
